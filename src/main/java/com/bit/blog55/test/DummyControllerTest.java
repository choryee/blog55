...
import com.bit.blog55.repository.UserRepository;
...
//	위와 같은 결과인데, 파라미터를 객체를 넣어도 결과 동일.
	public String join(User user) {
		...
		//user.setRole("user"); //이렇게 하면, 유지보수하는 개발자가 입력값을 잘못 입력할수 있으므로, enum으로 만드는것.
...
//			@Override
//			public User get() {
//				return new User(); //4번 넣으면, 빈 객체라도 리턴해줌. 최소 널을 리턴하지 않음.
//			}
//		});
...
//		});
		...
		//위를 람다식으로
		User user=userRepository.findById(id).orElseThrow(()->{
			//orElseThrow안에 어떤 타입을 써야 하지는 신경안쓰고 처리하므로, 위 보다 쉬움. 그게 람다식문법.
